...
void Account_private::setSecondname(string value)
{
	secondname = value;
}

bool Account_private::setNo(int value)
{
	if (value > 0)
	{
		no = value;
...
bool Account_private::setPercent(double value)
{
	if (value > 0)
	{
		percent = value;
...
Account_private::Account_private(int no, double percent)
{
	setNo(no);
	setPercent(percent);
}
Account_private::Account_private(const Account_private& a)
{
	setNo(a.getNo());
	setPercent(a.getPercent());
}

ostream& operator <<(ostream& out, const Account_private& x)
{
	out << (string)x;
	return out;
}
istream& operator >>(istream& in, Account_private& x)
{
	SetConsoleCP(1251);
...
Account_private& Account_private::operator =(const Account_private& r)
{
...
Account_private::operator string() const
{
	stringstream ss;
	ss << "----------------------------------------------------------------------------\n";
...
void Account_private::ChangeOwner()
{
	cin >> *this;
	setSumma();
	setPercent(5);
...
Account_private Account_private::MakePercent(Account_private& a)
{
	SetConsoleCP(1251);
	SetConsoleOutputCP(1251);
	double sum = 0;
...
Account_private Account_private::ToDollar(Account_private& a)
{
	SetConsoleCP(1251);
	SetConsoleOutputCP(1251);
	double sum = 0;
...
Account_private Account_private::ToEuro(Account_private& a)
{
	SetConsoleCP(1251);
	SetConsoleOutputCP(1251);
	double sum = 0;
...
double Account_private::Total(const Account_private& a) {
	int hrn, kop;
	double sum;
...
Account_private Account_private::Add(const Account_private& x, const Account_private& y, Account_private& a)
{
...
Account_private Account_private::Subtract(const Account_private& x, const Account_private& y, Account_private& a)
{
	double subt = Total(x) - Total(y);
	if (subt <= 0)
	{
		cerr << "Sum on first Account_private is lesser than that on the latter.\n";
		return a;
	}
	else
	{
...
Account_private Account_private::Multiply(Account_private& x, const double y)
{
	if (y <= 0)
		Account_private(x);
	else
...
Account_private Account_private::Division(Account_private& x, const double y)
{
	if (y == 0)
		Account_private(x);
	else
...
Account_private Account_private::DivOfSums(const Account_private& x, const Account_private& y, Account_private& a)
{
	double subt = Total(x) / Total(y);
	int hrn = (int)subt;
	int coin = (floor((subt - hrn) * 100 + 0.501));
...
Account_private Account_private::Change(int hrn, int coin, Account_private& d)
{
	d.setH500(hrn / 500);
	hrn -= d.getH500() * 500;
	d.setH200(hrn / 200);
...
bool operator < (Account_private a, Account_private b) {
...
bool operator != (Account_private a, Account_private b) { return !operator ==(a, b); }